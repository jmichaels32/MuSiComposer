...
def get_icl_prompts(
    ...
    **Note**: Be sure to shuffle the support examples and labels
    ...
    ### START CODE HERE ###
    shuffled_support_inputs = [support_inputs[i] for i in permutation]
    shuffled_support_labels = [support_labels[i] for i in permutation]
    if prompt_mode == "babi":
        key_word = " In the "
        for index, support_input in enumerate(shuffled_support_inputs):
            prompt += support_input
            prompt += key_word + shuffled_support_labels[index] + ". "
        prompt += test_input + " In the"
    elif prompt_mode == "none":
        for index, support_input in enumerate(shuffled_support_inputs):
            prompt += support_input + " " + shuffled_support_labels[index] + " "
        prompt += test_input
...
def do_sample(
    ...
    while len(sampled_tokens) < max_tokens:
        with torch.inference_mode():
            ...
            # Calculate the predicted token using greedy (take the max and then the positional prediction)
            most_recent_token = torch.argmax(logits, dim=-1)[0, -1]
            if most_recent_token in stop_tokens:
                break
            else:
                sampled_tokens.append(most_recent_token)
